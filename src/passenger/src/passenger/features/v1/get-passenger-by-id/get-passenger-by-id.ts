...
import { PassengerIdQueryDto } from '@/passenger/dtos/passenger-id-query.dto';
...
export class GetPassengerByIdController {
  ...
  public async getPassengerById(
    @Query() query: PassengerIdQueryDto,
...
export class GetPassengerByIdHandler implements IQueryHandler<GetPassengerById> {
  ...
  async execute(query: GetPassengerById): Promise<PassengerDto> {
    ...
    if (!query.isAdmin && passengerEntity.userId !== query.currentUserId) {
