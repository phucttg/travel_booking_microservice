...
@Injectable()
export class BookingOutboxDispatcherService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    this.runtimeHealthService.setComponentStatus('outbox', 'up', {
      message: 'Booking outbox dispatcher started'
    });
  ...
  private async dispatchPendingMessages(): Promise<void> {
    ...
    try {
      ...
      Logger.error('Booking outbox dispatcher failed', error as Error);
      this.runtimeHealthService.setComponentStatus('outbox', 'degraded', {
